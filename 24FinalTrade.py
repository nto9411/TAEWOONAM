...
def enhanced_buy_coin_v2(access_key, secret_key, coin):
    upbit = pyupbit.Upbit(access_key, secret_key)
    ...
    if krw_balance >= 5000:
        ...
        # If the response is None or unexpected, we directly proceed with the known coin symbol
        if response is None or not isinstance(response, dict):
            # Directly create a buy order for the given coin symbol
            response = upbit.buy_market_order(coin, krw_balance)
...
def sell_coin(access_key, secret_key, coin):
    ...
    if coin_balance and coin_balance > 0:
        response = upbit.sell_market_order(coin, coin_balance)
        
        # If the response is None or unexpected, we directly proceed with the known coin symbol
        if response is None or not isinstance(response, dict):
            # Directly create a sell order for the given coin symbol
            response = upbit.sell_market_order(coin, coin_balance)
    ...
    try:
        ...
        while True:
            ...
            if coin_prices[max_growth_coin][-1] > coin_prices[max_growth_coin][-2]:
                ...
                enhanced_buy_coin_v2(access_key, secret_key, max_growth_coin)
