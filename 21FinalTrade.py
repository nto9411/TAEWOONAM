...
def get_coin_price_and_volume(market):
    try:
        ...
        # After 3 tries, send an error message
        send_discord_message(f"Error fetching price and volume for {market}: Couldn't fetch data after 3 tries.")
...
def enhanced_buy_coin(access_key, secret_key, coin):
    ...
    if krw_balance is None:
        send_discord_message(f"Failed to fetch KRW balance for buying {coin}.")
        return
        ...
        while True:
            ...
            for _ in range(3):  # Loop for 3 times
                with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                    results = list(executor.map(get_coin_price_and_volume, krw_coins))
