...
def enhanced_buy_coin_v2(access_key, secret_key, coin):
    ...
    if krw_balance >= 5000:
        ...
        # Check if the response is None or unexpected type
        if response is None or not isinstance(response, dict):
            send_discord_message(f"Unexpected response when trying to buy {coin}.")
            return
    ...
    try:
        ...
        while True:
            ...
            while True:
                ...
                price, _ = safe_get_coin_price_and_volume_v3(max_growth_coin)
                if price:
