...
def safe_check_sell_condition(coin, buy_price, coin_prices, coin_volumes):
    ...
    if coin not in coin_prices or len(coin_prices[coin]) < 2:
        send_discord_message(f"Insufficient data for {coin} to check sell condition.")
        return False

    # 가격이 2% 이하로 하락했는지 확인
    if coin_prices[coin][-1] <= buy_price * 0.98:
        send_discord_message(f"{coin}'s price dropped by more than 2%. Selling condition met.")
        return True

    # 가격이 2% 이상으로 상승했는지 확인
    if coin_prices[coin][-1] >= buy_price * 1.02:
        send_discord_message(f"{coin}'s price rose by more than 2%. Selling condition met.")
        return True
    ...
    if coin in coin_volumes and check_volume_increase(coin, coin_volumes) and check_price_drop(coin, buy_price, coin_prices):
        send_discord_message(f"{coin} showed a 1.6x volume increase and 1% price drop. Selling condition met.")
        return True
    
    send_discord_message(f"Checked selling condition for {coin}. No conditions met yet.")
    return False
...
def updated_main_v2():
    try:
        ...
        send_discord_message("현재 잔고: ", krw)
